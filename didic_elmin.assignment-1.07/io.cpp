...
void io_poke_battle()
{
  ...
  double man_distance = abs(world.cur_idx[dim_x] - 200) + abs(world.cur_idx[dim_y] - 200);

  int IVhp = rand() % 15;
  int IVattack = rand() % 15;
  int IVdefense = rand() % 15;
  int IVspecialattack = rand() % 15;
  int IVspecialdefense = rand() % 15;
  int IVspeed = rand() % 15;
...
int start = 0;
    ...
    for( start = 0; start < 10194; start++)
    {
      if(stats[start].pokemon_id == randa)
      break;
    }
    ...
    int basehp = stats[start].base_stat;
    int baseattack = stats[start + 1].base_stat;
    int basedefense = stats[start + 2].base_stat;
    int basespecialattack = stats[start + 3].base_stat;
    int basesepcialdefense = stats[start + 4].base_stat;
    int basespeed = stats[start + 5].base_stat;

    int hp = (((basehp + IVhp) * 2) * level) / 100 + level + 10;
    int attack = (((baseattack + IVattack) * 2 * level) / 100 + 5);
    int defense = (((basedefense + IVdefense) * 2 * level) / 100 + 5);
    int special_attack = (((basespecialattack + IVspecialattack) * 2 * level) / 100 + 5);
    int special_defense = (((basesepcialdefense + IVspecialdefense) * 2 * level) / 100 + 5);
    int speed = (((basespeed + IVspeed) * 2 * level) / 100 + 5);
    ...
    for(int i = 0; i < 528238; i++) {
        if(pokemon_moves[i].pokemon_id == pokemon[randa].species_id) {
    ...
    for(int i = moves_start; i < 528238; i++) {
        if(pokemon_moves[i].pokemon_id != pokemon[randa].species_id) {
...
int  move_two_id = 0;
    ...
    for(int i = moves_start; i < moves_end; i++) {
        if(pokemon_moves[i].pokemon_move_method_id == 1 && pokemon_moves[i].level <= level && pokemon_moves[i].level != 0) {
             move_one_id = pokemon_moves[i].move_id;
             break;
        }
    }
    ...
    for(int i = moves_start; i < moves_end; i++) {
        if(pokemon_moves[i].pokemon_move_method_id == 1 && pokemon_moves[i].level <= level && pokemon_moves[i].level != 0) {
               move_two_id = pokemon_moves[i].move_id + 1;
 ...
 if(rand() % 8192 == 0)
 {
  ...
  mvprintw(0,0, "A wild %s appeard!", pokemon[randa].identifier);
  mvprintw(1,0, "level: %d", level);
  mvprintw(2,0, "Move 1: %s", moves[move_one_id].identifier);
  mvprintw(3,0, "Move 2: %s", moves[move_two_id].identifier);
  mvprintw(4,0, "id: %d", pokemon[randa].id);
  mvprintw(5,0, "is shiny: %s", shiny ? "true" : "false");
  mvprintw(6,0, "gender: %s", gender ? "female" : "male");
  mvprintw(7,0, "HP: %d  IV %d", hp, IVhp);
  mvprintw(8,0, "Attack: %d  IV %d", attack, IVattack);
  mvprintw(9,0, "Defense: %d IV %d", defense, IVdefense);
  mvprintw(10,0, "Special Attack: %d IV %d", special_attack, IVspecialattack);
  mvprintw(11,0, "Special Defense: %d IV %d", special_defense, IVdefense);
  mvprintw(12,0, "Speed: %d IV %d", speed, IVspeed);
